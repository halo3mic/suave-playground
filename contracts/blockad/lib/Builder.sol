...
contract Builder is EthBlockBidContract, SuaveContract {
	...
	) onlyConfidential public virtual override returns (bytes memory) {
	...
	function submitBlock() onlyConfidential external view returns (bytes memory) {
		...
		submitToRelay(builderBid);
        return abi.encodeWithSelector(this.buildAndEmitCallback.selector, new bytes(0), keccak256(builderBid));
	}

	function submitToRelay(bytes memory builderBid) internal view {
		(bool success, bytes memory data) = Suave.SUBMIT_ETH_BLOCK_BID_TO_RELAY.staticcall(abi.encode(boostRelayUrl, builderBid));
		if (!success) {
